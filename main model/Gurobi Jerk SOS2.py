...
- --input: CSV of per-timestep records.
  Required columns:
    * timestep_time
    * vehicle_id
    * vehicle_speed            (m/s)
    * vehicle_pos              (meters, absolute position used as odometer origin)
    * following_headway_distance   (meters)
    * preceding_headway_distance   (meters)
  Condition identifier:
    * Either 'condition_id' or Chinese '工况ID' (the script will auto-rename to 'condition_id').
  Optional but kept if present:
    * vehicle_type, vehicle_x, vehicle_y

- --accel-envelope: CSV describing the acceleration envelope versus speed.
  Expected columns:
    * speed_kmh   (integer km/h bins)
    * q1_smooth   (Amin at that speed, m/s^2)
    * q99_smooth  (Amax at that speed, m/s^2)

- --jerk-envelope: CSV describing jerk bounds versus acceleration grid.
  Expected columns:
    * accel_round (acceleration grid value, m/s^2)
    * q1_smooth   (JerkMin at that accel, m/s^3)
    * q99_smooth  (JerkMax at that accel, m/s^3)

Common options:
- --dt                 Timestep in seconds used when integrating distance (default 1.0).
- --output-dir         Directory for final CSV (default: ./outputs).
- --output-stem        Final CSV name stem without extension (default: dynamics_constrained).
- --tmp-dir            Directory for intermediate chunk files (default: ./outputs/tmp).
- --chunk-format       csv | parquet for intermediate parts (default: csv).
- --batch-size         Number of groups buffered before writing a part file (default: 500).
- --max-workers        Parallel worker processes (default: os.cpu_count()).
- --time-limit         Gurobi time limit per task in seconds (default: 600).
- --threads            Gurobi threads per task (default: 2).
- --save-plots         (Optional) Directory to save per-vehicle/condition speed plots (.png).
...
-------
1) Final dataset:
   {output-dir}/{output-stem}.csv
   Columns (only those available are kept):
     - vehicle_id
     - timestep_time
     - condition_id
     - vehicle_type
     - vehicle_odometer              (meters; first position + ∑ v*dt)
     - vehicle_speed                 (m/s; baseline)
     - vehicle_x, vehicle_y          (if present in input)
     - opt_jerk, opt_a, opt_v, opt_x (optimized jerk/acc/speed/position)

2) Intermediate parts (for large runs):
   {tmp-dir}/part_00000.{csv|parquet}, part_00001..., each containing:
     - timestep_time, vehicle_id, maxSpeed, opt_a, opt_v, opt_x, opt_jerk, base_v

3) Optional plots (if --save-plots is set):
   {save-plots}/{vehicle_id}_{condition_id}.png
   Line chart comparing baseline speed vs. optimized speed over time.

Units
-----
- Speed: m/s (plots show km/h for readability)
